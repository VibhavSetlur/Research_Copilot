...
│          │                                               │
│  ┌───────▼────────────────────────────────────────────┐  │
│  │  Tool Implementations (tools/actions/)             │  │
│  │  ┌─────────────────────┐ ┌─────────────────────┐   │  │
│  │  │ System (sys.*)      │ │ Tools (tool.*)      │   │  │
│  │  │                     │ │                     │   │  │
│  │  │ path.create         │ │ search.pubmed       │   │  │
│  │  │ path.abandon        │ │ python.exec         │   │  │
│  │  │ guidance.get        │ │ data.sample         │   │  │
│  │  │ file.read           │ │ web.scrape          │   │  │
│  │  └─────────────────────┘ └─────────────────────┘   │  │
│  └────────────────────────────────────────────────────┘  │
│                                                          │
│  ┌────────────────────────────────────────────────────┐  │
...
│  │  - Numbered experiment path creation               │  │
...
|----------|--------|---------------|---------|
| System | `sys.` | Control the OS itself | `sys.path.create`, `sys.file.read`, `sys.guidance.get` |
...
Experiments are created as `workspace/01_name/`, `02_name/`, etc. by `sys.path.create`. Each folder is self-contained with README.md, conclusions.md, data/, scripts/, outputs/, and environment/.
