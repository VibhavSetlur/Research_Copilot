...
Programmatic usage:
    from research_copilot.utils.session_restorer import get_restoration_prompt
...
def get_restoration_prompt(root: Optional[Path] = None, max_tokens: int = 800) -> str:
    ...
    if root is None:
        ...
        if root is None:
            return "ERROR: Not in a Research Copilot workspace. Run `rcp init` first."
    ...
    prompt_parts = [
        "# Context Restoration — Research Copilot",
        "",
