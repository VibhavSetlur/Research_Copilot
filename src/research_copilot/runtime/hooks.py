...
def citation_interceptor(state: Dict[str, Any], **kwargs) -> Dict[str, Any]:
    """Intercepts generated text and logs citations to workspace/manuscript/references.bib."""
    ...
    if citations:
        try:
            ...
            bib_path = resolved_root / "workspace" / "manuscript" / "references.bib"
            ...
            for citation in citations:
                # Basic parsing "Author, Year" -> key "AuthorYear"
                parts = citation.split(',')
                if len(parts) >= 2:
                    author = parts[0].strip().replace(" ", "")
                    year = parts[1].strip()
                    key = f"{author}{year}"
                    ...
                    if key not in existing_bib:
                        entry = (
                            f"@misc{{{key},\n"
                            f"  author = {{{parts[0].strip()}}},\n"
                            f"  year = {{{year}}},\n"
                            f"  title = {{Extracted Citation: {citation}}},\n"
                            f"  note = {{Autogenerated by Research OS Citation Interceptor}}\n"
                            f"}}\n"
                        )
