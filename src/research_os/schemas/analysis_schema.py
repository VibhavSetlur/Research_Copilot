...
class StatisticalTest(BaseModel):
    ...
    test_name: str = Field(
        ...,
        description="Name of statistical test (e.g., 't-test', 'ANOVA', 'OLS regression')",
    )
    statistic: float = Field(..., description="Test statistic value")
    degrees_of_freedom: Optional[float] = Field(
        default=None, description="Degrees of freedom"
    )
    p_value: float = Field(
        ..., ge=0, le=1, description="Exact p-value (not thresholded)"
    )
    effect_size: float = Field(..., description="Effect size estimate")
    effect_size_type: str = Field(
        ...,
        description="Type of effect size (cohens_d, r, eta_squared, odds_ratio, etc.)",
    )
    ...
    assumptions_checked: List[str] = Field(
        default=[], description="List of assumptions verified"
    )
    runtime: Optional[str] = Field(
        default=None, description="Runtime used (python, r, bash, etc.)"
    )
    tool_id: Optional[str] = Field(default=None, description="Tool registry ID")
    container: Optional[str] = Field(
        default=None, description="Container used for execution"
    )
