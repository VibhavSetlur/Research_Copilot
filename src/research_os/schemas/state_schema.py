...
class TokenBudget(BaseModel):
    ...
    def remaining_must_match(cls, v: int, info) -> int:
        if "used" in info.data and "limit" in info.data:
            ...
            if v != expected:
                raise ValueError(
                    f"Remaining ({v}) must equal limit - used ({expected})"
                )
        return v
...
class ContextTransferMemo(BaseModel):
    ...
    ctm_id: str = Field(
        ..., description="Unique ID for this CTM (format: ctm_<timestamp>)"
    )
    phase: str = Field(..., description="Pipeline phase when CTM was generated")
    token_usage_pct: float = Field(
        ..., ge=0.9, le=1.0, description="Token budget percentage at generation (>=0.9)"
    )
    ...
    immediate_goals: List[str] = Field(
        default=[], description="Immediate tactical goals right before the cutoff"
    )
    partial_results: List[Dict[str, str]] = Field(
        default=[], description="Incomplete results or computations in progress"
    )
...
class DAGNode(BaseModel):
    ...
    node_id: str = Field(
        ...,
        description="Unique node ID (format: <script_name>_<iteration_id>_<run_index>)",
    )
    script_path: str = Field(..., description="Path to the script that was executed")
    iteration_id: Optional[str] = Field(
        default=None,
        description="Iteration ID if this was part of an iteration (e.g., 001, 002)",
    )
    depends_on: List[str] = Field(
        default=[], description="List of node_ids this execution depends on"
    )
    input_files: List[str] = Field(
        default=[], description="Input data files consumed by this script"
    )
    output_files: List[str] = Field(
        default=[], description="Output files produced by this script"
    )
    status: str = Field(
        default="pending",
        description="Execution status: pending, running, complete, failed",
    )
    timestamp: str = Field(..., description="ISO 8601 timestamp of execution")
    data_hash_in: Dict[str, str] = Field(
        default={}, description="SHA-256 hashes of input files at execution time"
    )
    data_hash_out: Dict[str, str] = Field(
        default={}, description="SHA-256 hashes of output files at execution time"
    )
...
class ExecutionDAG(BaseModel):
    ...
    nodes: Dict[str, DAGNode] = Field(
        default={}, description="All execution nodes keyed by node_id"
    )
    edges: List[Dict[str, str]] = Field(
        default=[], description="Directed edges: [{from: node_id, to: node_id}]"
    )
...
class BranchState(BaseModel):
    ...
    branch_id: str = Field(
        ...,
        description="Unique branch identifier (e.g., 'hypothesis_B', 'bayesian_approach')",
    )
    parent_branch: str = Field(
        default="main", description="Parent branch this was created from"
    )
    created_at: str = Field(..., description="ISO 8601 timestamp of branch creation")
    status: str = Field(
        default="active", description="Branch status: active, merged, abandoned"
    )
    hypothesis: str = Field(
        default="", description="Research hypothesis or exploration goal"
    )
    merge_commit: Optional[str] = Field(
        default=None, description="Merge commit ID if branch was merged"
    )
    merged_at: Optional[str] = Field(
        default=None, description="ISO 8601 timestamp of merge"
    )
...
class ResearchObject(BaseModel):
    ...
    supporting_nodes: List[str] = Field(
        default_factory=list, description="DAG nodes supporting this"
    )
    conflicting_nodes: List[str] = Field(
        default_factory=list, description="DAG nodes contradicting this"
    )


class Hypothesis(ResearchObject):
    status: str = Field(
        default="active", description="Status: active, validated, invalidated"
    )
...
class CritiqueObject(ResearchObject):
    target_object_id: str = Field(
        ..., description="The ID of the object being critiqued"
    )
...
class EpisodicMemory(BaseModel):
    ...
    trigger: str = Field(
        ..., description="What triggered this memory (e.g. workflow_completion, branch)"
    )
    summary: str = Field(..., description="Summary of the events")
    decisions_made: List[str] = Field(
        default_factory=list, description="Key decisions made during this episode"
    )
    rejected_alternatives: List[str] = Field(
        default_factory=list, description="Paths considered but abandoned"
    )
...
class SemanticMemory(BaseModel):
    ...
    project_summary: str = Field(
        default="", description="Rolling summary of the entire project's current state"
    )
    confidence_evolution: str = Field(
        default="", description="Narrative of how confidence in hypotheses has changed"
    )


class MemoryState(BaseModel):
    model_config = ConfigDict(extra="forbid")
    """The multi-tiered memory architecture for the research os."""
    episodic: List[EpisodicMemory] = Field(
        default_factory=list, description="Chronological reasoning trace"
    )
    semantic: SemanticMemory = Field(
        default_factory=SemanticMemory, description="Compressed semantic knowledge"
    )
...
class ResearchState(BaseModel):
    ...
    research_objects: CognitiveObjects = Field(
        default_factory=CognitiveObjects,
        description="Semantic state of the research (hypotheses, claims, etc.)",
    )
    active_hypotheses: List[dict] = Field(
        default=[], description="Active hypotheses being tested"
    )
    dead_ends: List[str] = Field(
        default=[], description="Approaches tried and abandoned"
    )
    ...
    tool_availability: Dict[str, str] = Field(
        default={},
        description="Tool availability map: tool_id -> AVAILABLE/INSTALLABLE/MISSING",
    )
    ...
    data_scale_profile: Optional[Dict[str, str]] = Field(
        default=None,
        description="Data scale profile: {file_path: 'small'|'medium'|'large'|'massive'}",
    )
