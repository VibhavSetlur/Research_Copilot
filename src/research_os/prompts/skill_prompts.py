...
### CLI Reference
```bash
python -m research_os.utils.synthesize_results --results-file <results_json_path>
```
""",
...
### Python Helper: `research_os.utils.auto_debug`
```bash
python -m research_os.utils.auto_debug --script <path> --max-attempts 3
```
...
### CLI Reference
```bash
python -m research_os.utils.cache_manager --clear
python -m research_os.utils.cache_manager --stats
```
""",
...
```bash
python -m research_os.utils.parallel_runner --tasks <tasks_json_file> --max-workers <num>
```
...
```
CONTEXT TRANSFER — Research OS Session Continuation
...
1. **Resolve Library ID:**
   ...
   ```bash
   python -m research_os.utils.context7_lookup resolve <library_name>
   ```
...
2. **Query Documentation:**
   ...
   ```bash
   python -m research_os.utils.context7_lookup docs <library_id> <topic_or_function>
   ```
...
- CLI Command: `python -m research_os.utils.assumption_validator --data <path> --config <config_path>`
""",
    "github_release": """---
...
Execute `python -m research_os.utils.figure_validator --directory reports/figures/`
...
- Run via: `python -m research_os.utils.citation_verifier --bibliography reports/literature/bibliography.bib`
""",
...
# Or search by tags
items = zot.items(tags='research-os')
...
```python
def sync_to_zotero(bibliography, collection_name="Research OS"):
    ...
    for entry in bibliography:
        item = {
            ...
            'tags': [{'tag': 'research-os'}, {'tag': entry['relevance']}],
        }
...
```bash
python -m research_os.utils.figure_validator path/to/figure.png
```
