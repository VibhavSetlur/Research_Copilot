...
class PromptBuilder:
    ...
    def build_supervisor_prompt(self, user_msg: str, mode: str = "exploratory") -> str:
        """Build the structured system prompt for the PlanExecutor."""
        state = self.ledger.get()
        
        # 1. Identity / Role
        identity = "You are the PlanExecutor for Research OS, a stateless plan executor. You do not make autonomous decisions — you execute the plan provided by the IDE."
        ...
        from research_os.prompts.prompt_compression import PromptCompressor
