...
class HPCExecutor:
    ...
    def submit_remote_job(
        self, script_path: str, target: str, config: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        logger.info(
            f"Submitting {script_path} via profile {target} (engine: {engine}, container: {container})"
        )
