...
def check_dpi(img: Image.Image, minimum: int = 300) -> Tuple[bool, str]:
    ...
    if width >= 1200:
        return (
            True,
            f"DPI metadata missing, but resolution {width}x{height} is sufficient (assumed >= 300 DPI)",
        )
    return (
        False,
        f"DPI metadata missing and resolution {width}x{height} is too low for publication",
    )
...
def check_axis_origin_truncation(img: Image.Image) -> Tuple[bool, str]:
    ...
    if (clipped / total_border) > 0.005:
        return (
            False,
            f"Potential truncation detected: {clipped} pixels touch the outer image borders",
        )
...
def check_axis_labels_heuristic(img: Image.Image) -> Tuple[bool, str]:
    ...
    for y in range(int(height * 0.2), int(height * 0.8), 5):
        ...
        non_bg = sum(
            1 for p in row_colors if sum(abs(p[i] - bg_color[i]) for i in range(3)) > 30
        )
        if non_bg > 0:
            left_variance += 1
    ...
    for x in range(int(width * 0.2), int(width * 0.8), 5):
        col_colors = [
            rgb_img.getpixel((x, y))
            for y in range(height - bottom_margin_h, height - 5, 2)
        ]
        non_bg = sum(
            1 for p in col_colors if sum(abs(p[i] - bg_color[i]) for i in range(3)) > 30
        )
        if non_bg > 0:
            bottom_variance += 1
    ...
    if left_variance < 3:
        return (
            False,
            "Failed: Left margin lacks pixel variation. Y-axis label or ticks may be missing.",
        )
    if bottom_variance < 3:
        return (
            False,
            "Failed: Bottom margin lacks pixel variation. X-axis label or ticks may be missing.",
        )

    return (
        True,
        "Left and bottom margin variance check passed (labels/ticks likely present)",
    )
...
def check_colorblind_palette(img: Image.Image) -> Tuple[bool, str]:
    ...
    if has_red and has_green:
        return (
            False,
            "Warning: Image contains both saturated Red and Green, which is problematic for deuteranopia/protanopia. Use Okabe-Ito colors.",
        )

    return (
        True,
        "Color palette check passed (no high-contrast red-green pairings detected)",
    )
...
def validate_figure_file(path_str: str) -> Dict[str, Any]:
    ...
    try:
        ...
        with Image.open(path) as img:
            ...
            results["truncation"] = {
                "status": "PASS" if trunc_ok else "WARN",
                "message": trunc_msg,
            }
            if not trunc_ok:
...
def main():
    parser = argparse.ArgumentParser(
        description="Verify figure quality and layout compliance"
    )
