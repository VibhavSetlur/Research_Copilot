...
class AutomatedEDA:
    ...
    def run_eda(self, dataset_name: str) -> str:
        ...
        try:
            ...
            out_dir = self.root / "workspace" / "figures" / f"{Path(dataset_name).stem}_eda"
            ...
            if missing.sum() > 0:
                ...
                report_lines.append(f"Found missing values. Chart saved to `figures/{Path(dataset_name).stem}_eda/missing_values.png`\n")
            ...
            if len(numeric_cols) > 0:
                ...
                for col in numeric_cols[:10]:  # Limit to top 10 to avoid bloat
                    ...
                    plt.close()
                report_lines.append(f"Histograms generated for {len(numeric_cols[:10])} numeric columns.\n")
                
            # Compile report
            report_path = self.root / "workspace" / "data" / "derived" / f"{Path(dataset_name).stem}_eda_report.md"
