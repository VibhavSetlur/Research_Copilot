    ...
    try:
        ...
        try:
             ...
             if getattr(settings, "SERPAPI_API_KEY", None):
                  ...
                  formatted = [{"title": r.get("title", ""), "url": r.get("link", ""), "description": r.get("snippet", "")} for r in results]
                  return {"query": query, "source": "web (serpapi fallback)", "count": len(formatted), "results": formatted}
             else:
                  raise e
...
def scrape_web(url: str) -> Dict[str, Any]:
    try:
        ...
        return {"url": url, "content": "", "warning": "Firecrawl API key not set and trafilatura fallback failed"}
    except Exception as e:
